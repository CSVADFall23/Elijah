import java.util.*;

PGraphics testCanvas;
PGraphics refCanvas;
Population population;
int round = 0;
int difMax;
int selectCount;
int randomCount;
int geneSize;

int populationSize = 120;
int numShapes = 80;
int numVertices = 3;
float mutationChance = 0.01;
float mutationAmount = 0.15;
float selectionCutoff = 0.15;
int testRes = 80;
boolean staticRef = false;
boolean displayRef = true;
String saveName = "Checkerboard";

void setup() {
  size(640, 640);
  colorMode(RGB, 1, 1, 1, 1);
  noStroke();
  
  testCanvas = createGraphics(testRes, testRes);
  testCanvas.beginDraw();
  testCanvas.colorMode(RGB, 1, 1, 1, 1);
  testCanvas.noStroke();
  testCanvas.endDraw();
  refCanvas = createGraphics(testRes, testRes);
  refCanvas.beginDraw();
  refCanvas.colorMode(RGB, 1, 1, 1, 1);
  refCanvas.noStroke();
  refCanvas.endDraw();
  updateReference();
  
  difMax = width*height*3;
  selectCount = ceil(populationSize*selectionCutoff);
  randomCount = ceil(1/selectionCutoff);
  geneSize = 4+(2*numVertices);
  
  population = new Population();
  Individual best = population.bestChoice();
  best.display();
}

void draw() {
  if (!staticRef) {
    updateReference();
  }
  population.naturalSelection();
  Individual best = population.bestChoice();
  best.display();
  
  if (displayRef) {
    image(refCanvas, 0, 0);
  }
  
  round++;
  println(population.averageFit() + "    " + round);
}

class Population {
  Individual[] genePool;
  
  Population() {
    genePool = new Individual[populationSize];
    for (int i = 0; i < populationSize; i++) {
      genePool[i] = new Individual();
    }
    sortByFitness();
  }
  
  void naturalSelection() {
    Individual[] newGenePool = new Individual[selectCount*randomCount];
    for (int i = 0; i < selectCount; i++) {
      Individual p1 = genePool[i];
      for (int j = 0; j < randomCount; j++) {
        int index = i;
        while (index == i) {
          index = (int)random(populationSize);
        }
        Individual p2 = genePool[index];
        newGenePool[i*randomCount+j] = new Individual(p1, p2);
      }
    }
    for (int i = 0; i < populationSize; i++) {
      genePool[i] = newGenePool[i];
    }
    for (int i = 0; i < populationSize; i++) {
      if (genePool[i] == null) {
        println(i);
      }
    }
    sortByFitness();
  }
  
  void sortByFitness() {
    Arrays.sort(genePool, new Comparator<Individual>() {
      @Override
      public int compare(Individual a, Individual b) {
        return Float.compare(b.fitness, a.fitness);
      }
    });
  }
  
  Individual bestChoice() {
    return genePool[0];
  }
  
  float averageFit() {
    float total = 0;
    for (Individual ind : genePool) {
      total += ind.fitness;
    }
    total /= populationSize;
    return total;
  }
}

class Individual {
  float[] genes;
  float fitness;
  
  Individual() {
    genes = new float[numShapes*geneSize];
    for (int i = 0; i < genes.length; i += geneSize) {
      genes[i] = random(1);
      genes[i+1] = random(1);
      genes[i+2] = random(1);
      genes[i+3] = max(random(1)*random(1), 0.2);
      float x = random(1);
      float y = random(1);
      for (int j = 0; j < numVertices; j++) {
        genes[i+4+2*j] = x+random(-0.5, 0.5);
        genes[i+4+2*j+1] = y+random(-0.5, 0.5);
      }
    }
    setFitness();
  }
  
  Individual(Individual p1, Individual p2) {
    genes = new float[numShapes*geneSize];
    for (int i = 0; i < genes.length; i += geneSize) {
      if (random(1) < 0.5) {
        for (int j = 0; j < geneSize; j++) {
          genes[i+j] = p1.genes[i+j];
        }
      }
      else {
        for (int j = 0; j < geneSize; j++) {
          genes[i+j] = p2.genes[i+j];
        }
      }
    }
    for (int i = 0; i < genes.length; i++) {
      if (random(1) < mutationChance) {
        genes[i] += random(-mutationAmount, mutationAmount);
        genes[i] = constrain(genes[i], 0, 1);
      }
    }
    if (random(1) < 0.5) {
      for (int i = 0; i < genes.length-geneSize; i += geneSize) {
        if (random(1) < mutationChance) {
          for (int j = 0; j < geneSize; j++) {
            float temp = genes[i+j];
            genes[i+j] = genes[(i+geneSize)+j];
            genes[i+geneSize+j] = temp;
          }
        }
      }
    }
    setFitness();
  }
  
  void setFitness() {
    testCanvas.beginDraw();
    display(testCanvas);
    testCanvas.endDraw();
    
    testCanvas.loadPixels();
    refCanvas.loadPixels();
    float total = 0;
    for (int i = 0; i < refCanvas.pixels.length; i++) {
      total += pow(red(refCanvas.pixels[i])-red(testCanvas.pixels[i]),2);
      total += pow(green(refCanvas.pixels[i])-green(testCanvas.pixels[i]),2);
      total += pow(blue(refCanvas.pixels[i])-blue(testCanvas.pixels[i]),2);
    }
    fitness = 1 - total/difMax;
  }
  
  void display(PGraphics canvas) {
    canvas.beginDraw();
    canvas.background(0);
    for (int i = 0; i < genes.length; i += geneSize) {
      canvas.fill(genes[i], genes[i+1], genes[i+2], genes[i+3]);
      canvas.beginShape();
      for (int j = 0; j < numVertices; j++) {
        canvas.vertex(canvas.width*genes[i+4+2*j], canvas.height*genes[i+4+2*j+1]);
      }
      canvas.endShape();
    }
    canvas.endDraw();
  }
  
  void display() {
    background(0);
    for (int i = 0; i < genes.length-geneSize; i += geneSize) {
      fill(genes[i], genes[i+1], genes[i+2], genes[i+3]);
      beginShape();
      for (int j = 0; j < numVertices; j++) {
        vertex(width*genes[i+4+2*j], height*genes[i+4+2*j+1]);
      }
      endShape();
    }
  }
}

void updateReference() {
  //  Checkerboard
  refCanvas.beginDraw();
  refCanvas.background(0);
  int gridSize = 4;
  for (int i = -1; i < gridSize+1; i++) {
    for (int j = -1; j < gridSize+1; j++) {
      if ((i+j)%2 == 0) {
        refCanvas.fill(1, 0, 0);
      }
      else {
        refCanvas.fill(0, 0, 1);
      }
      refCanvas.rect(i*testRes/gridSize, j*testRes/gridSize, testRes/gridSize, testRes/gridSize);
    }
  }
  refCanvas.endDraw();
  
  //  Bars
  //refCanvas.beginDraw();
  //refCanvas.background(0);
  //int gridSize = 8;
  //for (int i = -1; i < gridSize+1; i++) {
  //  if (i%2 == 0) {
  //    refCanvas.fill(1, 0, 0);
  //  }
  //  else {
  //    refCanvas.fill(0, 0, 1);
  //  }
  //  refCanvas.rect(i*testRes/gridSize, 0, testRes/gridSize, testRes);
  //}
  //refCanvas.endDraw();
  
  //  Color noise
  //float scale = 0.005;
  //float speed = 0.01;
  //colorMode(HSB, 1, 1, 1);
  //refCanvas.loadPixels();
  //for (int i = 0; i < testRes; i++) {
  //  for (int j = 0; j < testRes; j++) {
  //    float h = 2*noise(i*scale, j*scale, 0+frameCount*speed);
  //    h %= 1;
  //    float b = 0.5*noise(i*scale, j*scale, 10+frameCount*speed)+0.5;
  //    color c = color(h, 1, b);
  //    refCanvas.pixels[i+j*testRes] = c;
  //  }
  //}
  //refCanvas.updatePixels();
  //colorMode(RGB, 1, 1, 1, 1);
  
  //  Floating ball
  //refCanvas.beginDraw();
  //refCanvas.background(0);
  //refCanvas.fill(1, 1, 1);
  //refCanvas.circle(testRes*mouseX/(float)width, testRes*mouseY/(float)height, 16);
  //refCanvas.endDraw();
}

void mouseClicked() {
  String name = "Genetic_" + saveName + "_" + (int)random(9999) + ".png";
  save(name);
  println("SAVED as " + name);
}
